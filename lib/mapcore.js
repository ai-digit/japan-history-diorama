...
/**
 * ビルド時に焼き込んだ標高格子。`bake-dem.mjs` が国土地理院の標高タイルから
...
/**
 * 立体模型として焼いてある天守の3類型。`models/castle-<型>.glb` と1対1で対応する。
 *   boro     望楼型 — 大きな入母屋の上に小さな望楼を載せる古式（安土・犬山・姫路）
 *   soto     層塔型 — 各階が均等に逓減して積み上がる新式（慶長後期以降・名古屋・寛永の江戸）
 *   hirajiro 平城   — 天守を持たない城。広い石垣と水堀が主役（二条・金沢）
 */
export const CASTLE_FORMS = ['boro', 'soto', 'hirajiro'];

/**
 * 層塔型が現れる線。藤堂高虎が慶長期の終わりに作った積み方で、これ以降に**設計された**
 * 天守は層塔型に寄る。**年そのものが型なのではなく、年が設計の系統の代理**である。
 */
export const SOTO_FROM_YEAR = 1610;

/**
 * 規則（下の `castleFormOf`）が **`keep_built` を「いま建っている天守の設計年」として読む**
 * ために外れる地点。外れ方は1種類しかない——**記録されている年が、より古い姿を写した
 * 後代の再建**である場合。再建天守はふつう創建の姿を写すので（名古屋の 1959 は 1612 の
 * 層塔型を、会津の 1965 は 1639 の層塔型を写していて、規則の答えと史実が一致する）、
 * 規則が外れるのは「創建が 1610 より前の望楼型なのに、記録された年が 1610 より後」の4件だけになる。
 *
 * **ここに並ぶ id は「データの誤り」ではない。** `keep_built` は再建年として正しく、
 * 型は別の事実である。本来は `sites.jsonl` に `keep_form` として持つべき史実で、
 * その1列が入れば下の例外表は消える（G-284 申し送り。この受け皿はデータ不可触のため
 * コード側に置いた）。
 */
export const CASTLE_FORM_EXCEPTIONS = {
  // 昭和6年(1931)の復興天守は、豊臣期の望楼型の意匠を写している
  osaka: 'boro',
  // 昭和35年(1960)の再建は、加藤清正が慶長6年(1601)に建てた望楼型大天守の姿
  kumamoto: 'boro',
  // 延享4年(1749)の再建は、創建(1601)の望楼型を踏襲して建てられた
  kochi: 'boro',
  // 琉球王国の正殿であって日本式の天守ではない。3類型では横広の平城がいちばん近い
  shuri: 'hirajiro',
};

/**
 * その地点をどの天守模型で描くか。**城として描かれる時だけ型を返す**。
 *
 * @param {object} site  `sites.jsonl` の1件（**読むだけ**。値には触れない）
 * @param {string} kind  その時代の姿（`kindAt()` の結果）。既定は地点の既定 kind
 * @returns {'boro'|'soto'|'hirajiro'|null}  城でなければ `null`＝**現行の手続き模型のまま**
 *
 * 【なぜ kind を引数に取るか】同じ地点でも時代で姿が変わる（G-184）。江戸城は明治で
 * `palace`、原城は江戸で `castle_ruin`、安土城は明治で `ruins` になる。**その時代に城で
 * ないものへ天守を建ててはいけない**ので、判定は「その時代の kind」に対して行う。
 * `castle` 以外（古墳・遺跡・神社・城跡…）は `null` を返し、呼ぶ側は手続き模型を使う。
 *
 * 【なぜ純粋関数か】ここが返す値は**絵の選択だけ**に効く。当たり判定(G-241)・譲り(G-11)・
 * 融合(SC-6)が読む `topY / viewFootR / footR` は `SHAPE_KEEP` から算き続ける
 * ——型を間違えても寸法は1ビットも動かない、という継ぎ目の作り方は G-67 で実測済み。
 */
export function castleFormOf(site, kind = site && site.kind) {
  if (!site || kind !== 'castle') return null;
  // **例外表は「自分で持っている鍵」だけを引く。** `CASTLE_FORM_EXCEPTIONS[site.id]` と書くと
  // id が `constructor` や `toString` の地点で Object.prototype の値が返り、この関数は
  // 3類型ではなく**関数を返す**（Codex 敵対レビュー G-284 #7）。返り値の型の約束は、
  // 呼ぶ側が `CASTLE_MODEL_URL[form]` を引くところで静かに undefined になって効いてくる。
  const forced = Object.prototype.hasOwnProperty.call(CASTLE_FORM_EXCEPTIONS, site.id)
    ? CASTLE_FORM_EXCEPTIONS[site.id] : null;
  if (forced) return forced;
  const built = site.keep_built;
  // 天守の記録が無い＝天守を持たない城。石垣と堀の平城で描く。
  // **数として読めない年もここへ倒す**（`NaN` / `"1609年"` / `""` / 真偽値）。
  // 素の比較に任せると `NaN < 1610` が false で層塔型、`"" < 1610` が true で望楼型になり、
  // **表記が揺れただけで型が反転する**（Codex #8/#9/#10）。読めない年から型を騙らない。
  // 「読めない年」がデータに入ったこと自体はデータ側の検査が赤にする——
  // 画面は安全に倒れ、混入は検査で止まる、という二段にしてある。
  if (typeof built !== 'number' || !Number.isFinite(built)) return 'hirajiro';
  return built < SOTO_FROM_YEAR ? 'boro' : 'soto';
}
...
/**
 * ラベルの優先度。
 * **選択 > ホバー > tier > その時代の中央政庁 > その時代に生きている > 手前** の順で効く。
 * 桁を分けてあるので、下位の項がどれだけ大きくても上位を逆転しない。
 *
 * `seat`（G-265・ムラ判定 `g265-tiebreak-ruling`）は
 * 「その時代に**中央政庁・王城として機能していた**か」＝データ側の `eras_seat` に当代が入るか。
 * これが無かったので、江戸期に生きている tier1 の10件が全て `3100.2` の同値になり、
 * **勝敗が `SITES` の配列順**で決まっていた（320x640 の江戸で江戸城が落ち、会津若松城が残る）。
 * 200 という値は「alive の最大 100 ＋ depth の最大 20 = 120 を上回り、tier の桁 1000 を下回る」
 * ——**tier の勝敗を seat が覆さない**（tier1 は常に tier2/3 に勝ち、tier1 の内側でだけ seat が効く）。
 *
 * @param {{tier?:number, alive?:number, depth?:number, seat?:boolean, selected?:boolean, hovered?:boolean}} a
 */
export function labelPriority({ tier = 2, alive = 0, depth = 0, seat = false, selected = false, hovered = false }) {
...
export function layoutLabels(items, { padding = 4, limit = Infinity } = {}) {
  // **同値の決着を入力の順序に委ねない**（G-265 層C・ムラ判定で必須と裁定）。
  // `Array.prototype.sort` は安定なので、比較が 0 を返した組は**渡された配列の順**
  // ＝ `sites.jsonl` の行順のまま残る。それは重要度でも画面上の位置でも時代との関係でもない。
  // `eras_seat` を足した後も同値は理論上また起こりうるので、**根そのものを塞ぐ**：
  // 勝敗は必ず「id の順」という、恣意的だが**説明できる・再現する**規則で決まる。
  //
  // `localeCompare` ではなく符号なしの比較にしてあるのは、`localeCompare` の結果が
  // 実行環境のロケール・ICU の版で変わりうるからである（id は `^[a-z][a-z0-9-]{1,31}$` で
  // ハイフンを許すが、ICU は既定でハイフンを弱く扱う＝`a-b` と `ab` の順が環境で入れ替わる）。
  // **環境で答えが変わる比較は、配列順依存を別の非決定性で置き換えるだけ**になる。
